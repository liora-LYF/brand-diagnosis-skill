...
- [ ] `<th>` 元素使用了荧光色背景（通过 `html-report.css` 的 `th` 样式）
...
1. **整体视觉效果**：白背景、黑色文字、荧光色（#C8FF00）高亮元素明确可见
...
| 打印时荧光色丢失 | 浏览器未启用背景打印 | 勾选"打印背景图形"/"背景颜色"。CSS 中已有 `print-color-adjust: exact` |
